CREATE OR REPLACE MACRO audit.check_cluster_006(epoch_start, epoch_end, p_max_delta := 500) AS TABLE
  WITH prev_epoch AS (
  ...
  deltas AS (
    SELECT ep.epoch, c.server_pk, c.connections - p.connections AS delta
    FROM epoch_pairs ep
