CREATE OR REPLACE MACRO audit.check_server_004(epoch_start, epoch_end) AS TABLE
  WITH prev_epoch AS (
  ...
  deltas AS (
    SELECT ep.epoch, c.server_pk, GREATEST(c.slow_consumers - p.slow_consumers, 0) AS delta
    FROM epoch_pairs ep
