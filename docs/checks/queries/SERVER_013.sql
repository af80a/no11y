CREATE OR REPLACE MACRO audit.check_server_013(epoch_start, epoch_end) AS TABLE
  WITH prev_epoch AS (
  ...
  deltas AS (
    SELECT ep.epoch, c.server_pk, GREATEST(c.stalled_clients - p.stalled_clients, 0) AS delta
    FROM epoch_pairs ep
